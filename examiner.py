...
questions = ['''請回答以下問題（提供選項數字即可，不用解釋）：
1. 根據本課程講座分享，資安新思維「永不信任，一律驗證」的概念是？
0: 零信任
1: 數位平等
2: 網路釣魚
3: 社交工程''',
'''請回答以下問題（提供選項數字即可，不用解釋）：
...
class E():
    ...
    def load_cache():
        try:
            with open('logs/examiner.pkl', 'rb') as fp:
    ...
    @staticmethod
    def save_cache():
        with open('logs/examiner.pkl', 'wb') as fp:
    ...
    def query(prompt: str) -> str:
        ...
        response = E.model.generate_content(prompt)
